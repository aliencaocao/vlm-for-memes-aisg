...
root = Path("thakkinapalli_memes_classification")
img_root = root / "memes_dataset"
...
def _process_single_image(img_path: Path) -> tuple[str, str]:
    # Skip all the images which are labelled as NOT memes
    category_name = img_path.parent.name
    if category_name == "not_meme":
        return None, None
...
if __name__ == "__main__":
    ...
    run(get_responses("thakkinapalli_memes_classification", processed, output_path=root / "labels.csv", is_async=True))
