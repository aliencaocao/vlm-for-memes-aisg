# Modified from llava_pipeline_eval.py
import atexit
import signal
import sys
import os
...
model = os.environ['MODEL_PATH'].split('/')[-1]

with open('llava_requests.json', 'rb') as f:
    llava_requests = orjson.loads(f.read())

output_fname = f'qwen_pipeline_output_{model}.json'
...
if __name__ == '__main__':
    ...
    server_manager.start_server('qwen')
    TOTAL_TOKENS = 0
    all_requests = []
    for request_body in tqdm(create_batch(llava_requests, llava_batch_size), desc='Qwen Inference'):
        ...
        while img_hard_retry_counter <= llava_max_hard_retry_per_batch:
            try:
                _res: list[dict] = server_manager.request('qwen', request_body)
                if _res:
                ...
                if all(success.values()):
                    ...
                    if img_hard_retry_counter >= llava_max_hard_retry_per_batch:
                        tqdm.write(f'[qwen] Inference failed for sample after {llava_max_hard_retry_per_batch} HARD retries', file=sys.stderr)
                        res += [x for x in _res if not success[x['im_path']]]
                        break
                    else:
                        if img_soft_retry_counter < llava_max_soft_retry_per_batch:
                            tqdm.write('[qwen] Inference failed for sample, SOFT retrying', file=sys.stderr)
                            img_soft_retry_counter += 1
                        else:
                            tqdm.write(f'[qwen] Inference failed for sample after {llava_max_soft_retry_per_batch} SOFT retries, HARD retrying', file=sys.stderr)
                            raise ForceRetryException()

            except (requests.exceptions.Timeout, ForceRetryException) as e:
                if isinstance(e, requests.exceptions.Timeout):
                    tqdm.write(f'[Qwen] Inference timeout for batch size {llava_batch_size}, HARD retrying', file=sys.stderr)
                img_soft_retry_counter = 0
                img_hard_retry_counter += 1

        if len(res) != len(request_body):
            tqdm.write(f'[qwen] Inference failed for {len(request_body) - len(res)} samples, skipping', file=sys.stderr)
        for r in res:
    ...
    print('qwen total tokens:', TOTAL_TOKENS, file=sys.stderr)

    server_manager.stop_server('qwen')
