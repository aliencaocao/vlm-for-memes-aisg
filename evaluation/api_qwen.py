...
processor = Qwen2VLProcessor.from_pretrained('Qwen/Qwen2-VL-7B-Instruct', local_files_only=False)
model = Qwen2VLForConditionalGeneration.from_pretrained(model_path, low_cpu_mem_usage=True, local_files_only=False, torch_dtype=torch.float16)
model = model.to(torch.device(f'cuda' if torch.cuda.is_available() else 'cpu'))
...
def pprint(x: str):
    print(f'[Qwen API] {x}', file=sys.stderr)
...
def process_request(request: InferenceRequest) -> tuple[str, Image.Image, str, bool]:
    ...
    return im_path, image, prompt, sample
...
def inference(requests: list[InferenceRequest]) -> list[dict[str, Union[str, bool, float]]]:
    ...
    def infer_batch(im_paths: list[str], images: list[Image.Image], prompts: list[str], sample: bool) -> list[dict[str, Union[str, bool, float]]]:
        try:
                ...
                messages = [
                    {
                        ...
                        "content": [
                            {
                                "type": "image",
                                "image": 'placeholder', # just for the chat template
                            },
            ...
            inputs = processor(text=prompts_batch, images=images, padding=True, return_tensors="pt").to(model.device)
            with sdap_backend():
