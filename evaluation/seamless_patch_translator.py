...
class Translator(nn.Module):
    ...
    def predict(
        self,
        input: Union[Tensor, SequenceData, list[str]],
        task_str: str,
        tgt_lang: str,
        ...
        :param input:
            Either list of text or path to audio or audio Tensor.
        :param task_str:
        ...
        if isinstance(input, dict):
            ...
            # commented out for batched inference
            # text = input
            # assert isinstance(text, str)

            assert isinstance(input, list)  # added for batched inference

            self.token_encoder = self.text_tokenizer.create_encoder(
                task="translation", lang=src_lang, mode="source", device=self.device
            )
            # src = self.collate(self.token_encoder(text))  # commented out for batched inference
            results = []
            for text in input:
                results.append(self.collate(self.token_encoder(text)[:1024]))  # at most 1024 tokens so we truncate

            src = {
                'is_ragged': False,
                'seqs': pad_sequence([torch.squeeze(result['seqs']) for result in results], batch_first=True, padding_value=self.text_tokenizer.vocab_info.pad_idx),
                'seq_lens': torch.cat([result['seq_lens'] for result in results], dim=0)
                }


        # assert isinstance(self.model, UnitYModel)  # commented out as after torch.compile it will fail here
