...
def mod_ckpt(config, ckpt, bs):
    for i, line in enumerate(config):
        if line.startswith('adapter_name_or_path'):
            config[i] = f'adapter_name_or_path: {ckpt}'
        elif line.startswith('run_name'):
            config[i] = f'run_name: {"/".join(ckpt.split("/")[1:])}_BS_{bs}'
        elif line.startswith('per_device_eval_batch_size'):
            config[i] = f'per_device_eval_batch_size: {bs}'
    return config
    ...
    while not job_queue.empty():
        if not available_gpus.empty():
            with gpu_lock:
                ...
                bs = job_queue.get()
                if bs >= 12 and gpu_id != 0:
                    logging.info(f"Skipping BS {bs} on GPU{gpu_id}")
                    available_gpus.put(gpu_id)
                    job_queue.put(bs)
                    continue
                logging.info(f"Enqueuing BS {bs} on GPU{gpu_id}")
                proc = Process(target=run_job, args=(gpu_id, ckpt, bs))
                proc.start()
