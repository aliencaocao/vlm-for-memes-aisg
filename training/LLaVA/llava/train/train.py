...
__import__("liger_kernel.transformers").transformers.apply_liger_kernel_to_llama(rope=True, swiglu=True, cross_entropy=False, fused_linear_cross_entropy=True, rms_norm=True)
from embed_ipython import embed_ipython
...
from llava.constants import DEFAULT_IMAGE_TOKEN, DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN, IGNORE_INDEX
...
class DataArguments:
    ...
    eval_data_path: str = field(default='../training/test.json',
                           metadata={"help": "Path to the evaluation data, images should be in same folder as image_folder."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    eval_with_sampling: bool = field(default=False)
    eval_before_training: bool = field(default=False)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0] and instances[0]['image'] is not None:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images): # this is analogous to the last few lines in process_images of mm_utils.py
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    eval_dataset = LazySupervisedDataset(tokenizer=tokenizer,
                                ...
                                data_args=data_args) if data_args.eval_data_path else None
...
def train(attn_implementation=None):
    ...
    # training_args.load_best_model_at_end = True  # since we are saving every single checkpoint no need for this
    training_args.metric_for_best_model = 'auroc'
    training_args.greater_is_better = True
    # in our case bf16 is true so compute_dtype = torch.bf16
    compute_dtype = (torch.float16 if training_args.fp16 else (torch.bfloat16 if training_args.bf16 else torch.float32))

    bnb_model_from_pretrained_args = {}
    if training_args.bits in [4, 8]: # we skipping. training_args.bits should be left at default of 16 for us.
    ...
    # indeed we DO have vision tower and it is set to openai/clip-vit-large-patch14-336
    if model_args.vision_tower is not None:
        ...
        else: # this, we using this because its llava-v1.6-mistral-7b
    ...
    else: # skip, we do indeed have a vision tower
    ...
    if model_args.freeze_backbone: # skip, model_args.freeze_backbone shd be left as default False
        model.model.requires_grad_(False)

    if training_args.bits in [4, 8]: # skip
        from peft import prepare_model_for_kbit_training
        model.config.torch_dtype=(torch.float32 if training_args.fp16 else (torch.bfloat16 if training_args.bf16 else torch.float32)) # TODO i suspect there is a bug here
        model = prepare_model_for_kbit_training(model, use_gradient_checkpointing=training_args.gradient_checkpointing)

    if training_args.gradient_checkpointing: # yes true for us
    ...
    if training_args.lora_enable: # yes true for us
        ...
        if training_args.bits == 16: # yes true for us
            if training_args.bf16: # true for us
                model.to(torch.bfloat16) # we will be using bf16
    ...
    else: # we use this because its llava-v1.6-mistral-7b
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
            model_max_length=training_args.model_max_length,
    ...
    elif model_args.version == "v0.5": # nope
        tokenizer.pad_token = tokenizer.unk_token
    else: # we use this because version is v1
        tokenizer.pad_token = tokenizer.unk_token
        conversation_lib.default_conversation = conversation_lib.conv_templates["mistral_instruct"]

    if model_args.vision_tower is not None: # true for us because we do have vision tower
        ...
        # vision tower will become bf16
        vision_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device) # TODO shouldnt this look like L867???

        data_args.image_processor = vision_tower.image_processor
        data_args.is_multimodal = True

        model.config.image_aspect_ratio = data_args.image_aspect_ratio # here should be set to anyres
        model.config.image_grid_pinpoints = data_args.image_grid_pinpoints # here we put in our hardcoded string of the image grid points
        model.config.tokenizer_padding_side = tokenizer.padding_side
        model.config.tokenizer_model_max_length = tokenizer.model_max_length

        model.config.tune_mm_mlp_adapter = training_args.tune_mm_mlp_adapter = model_args.tune_mm_mlp_adapter # by default this is false and we dont seem to touch it
        if model_args.tune_mm_mlp_adapter: # skip
            model.requires_grad_(False)
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = True

        model.config.freeze_mm_mlp_adapter = training_args.freeze_mm_mlp_adapter # also false by default
        ...
        model.config.mm_use_im_start_end = data_args.mm_use_im_start_end = model_args.mm_use_im_start_end # we set to False
        model.config.mm_projector_lr = training_args.mm_projector_lr # our config specifies as 2e-5
        training_args.use_im_start_end = model_args.mm_use_im_start_end # this seems like redundant copy of 2 lines above
        model.config.mm_use_im_patch_token = model_args.mm_use_im_patch_token # we set to False
    ...
    close_inst_seq = [28748, 16289, 28793]
    yes_tokens = [i for i in range(tokenizer.vocab_size) if tokenizer.decode(i).lower().strip() == 'yes']  # all possible yes tokens
    yes_tokens_tensor = torch.tensor(yes_tokens, dtype=torch.int64, device=training_args.device)
    no_tokens = [i for i in range(tokenizer.vocab_size) if tokenizer.decode(i).lower().strip() == 'no']  # all possible no tokens
    no_tokens_tensor = torch.tensor(no_tokens, dtype=torch.int64, device=training_args.device)
    yes_no_tokens = yes_tokens + no_tokens
    yes_no_tokens_tensor = torch.tensor(yes_no_tokens, dtype=torch.int64, device=training_args.device)


    @torch.inference_mode()
    def generate_eval(self: LLaVATrainer,
        ...
        def get_classification_score(tokens: list[int], pred_scores: list[torch.FloatTensor]) -> float:
            ...
            score = softmax_scores[yes_no_idx].index_select(-1, yes_tokens_tensor if label_bool else no_tokens_tensor).sum() / softmax_scores.index_select(1, yes_no_tokens_tensor).sum()
            if not label_bool:
                score = 1 - score
